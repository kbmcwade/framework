...
public class FeedsDeepSightIPBot extends MethLib{

    
    String testName = "FeedsDeepSightIPBot";
    ...
    @Test
    public void testFeedsDeepSightIPBot () throws InterruptedException, SQLException, IOException, ParseException{
        ...
        List<Node> botNames = new ArrayList<Node>();
        String[] botNameAttribute = new String[2];
        ...
        indicatorSource = "DeepSight Advanced IP Reputation Bot XML Feed";
        ...
        botNameAttribute[0] = "DeepSight Intelligence Bot Name";
        ...
        for (Node node : indicators) {
            ...
            //parse bot names node
            botNames = node.selectNodes("/bot_names/bot_name");
           
            //loop through the bot names record nodes
            for (Node bnNode : botNames) {
               
                //parse bot name details
                botNameAttribute[1] = bnNode.valueOf("@name");
              
                //check db for bot name attributes
                for(int a=0; a<botNameAttribute.length;a++){
                      
                    //verify non-blank attribute values
                    if (!StringUtils.isBlank(botNameAttribute[1])){
                      
                        //check db for indicator attribute
                        if (!MethLib.dbQueryAttributeExists(indicator, indicatorSource, botNameAttribute[0], botNameAttribute[1])){
                      
                            appLogs.debug("FAILED - Attribute \'"+botNameAttribute[0]+": "+botNameAttribute[1]+"\' was not found for \'"+indicator+"\'.");
                           softAssert.fail();
